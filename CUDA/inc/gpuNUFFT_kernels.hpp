 ...
 *
 * Performs the adjoint gridding convolution step on the GPU, thus the interpolation 
 * from non-uniform sampled k-space data onto the uniform oversampled grid. 
 *  
 * The distance from each sample to its neighboring grid positions is computed and the corresponding 
 * data value is weighted by the kernel function according to the distance.
 *
 * CUDA function prototype.
 * 
 * 
 * @param data_d            Input k-space sample data value, complex, sorted due to precomputation
 * @param crds_d            k-space sample coordinate (non-cartesian), linearized array (x1,x2,x3,...,xn,y1,y2,y3,...,yn,z1,z2,z3,...zn)
 * @param gdata_d           Output k-space grid (cartesian)
 * @param kernel_d          precomputed interpolation kernel
 * @param sectors_d         precomputed data-sector mapping, defines the range of data elements per sector, e.g. 0,3,4,4,10 -> maps data points 0..3 to sector id 0, 3..4 to sector 1, no data point to sector 2, 4..10 to sector 3 an so on 
...
/**
 * \brief Adjoint gridding convolution implementation on GPU using sector load balancing. 
 *
 * Performs the adjoint gridding convolution step on the GPU, thus the interpolation 
 * from non-uniform sampled k-space data onto the uniform oversampled grid. 
 *  
 ...
 * data value is weighted by the kernel function according to the distance.
 *
 * In order to balance the work load per thread block a sector processing order is precomputed. 
 * 
 * CUDA function prototype.
 * 
 * 
...
/**
 * \brief Adjoint gridding convolution implementation on GPU using textures for kernel lookup. 
 *
 * Performs the adjoint gridding convolution step on the GPU, thus the interpolation 
 * from non-uniform sampled k-space data onto the uniform oversampled grid. 
 *  
...
/**
 * \brief Adjoint gridding convolution implementation on GPU using textures and sector load balancing. 
 *
 * Performs the adjoint gridding convolution step on the GPU, thus the interpolation 
 * from non-uniform sampled k-space data onto the uniform oversampled grid. 
 * 
 ...
 * data value is weighted by the kernel function according to the distance.
 * 
 * The kernel lookup is performed by the use of gpu textures and the workload is balanced. 
 * 
 * CUDA function prototype.
 *  
 * @param data_d            Input k-space sample data value, complex, sorted due to precomputation
 ...
 *
 * Performs the forward gridding convolution step on the GPU, thus the interpolation 
 * from uniform oversampled grid positions to non-uniform sampled k-space data points. 
 * 
 * The distance from each sample to its neighboring grid positions is computed and the corresponding 
 * data value is weighted by the kernel function according to the distance.
 * 
 * CUDA function prototype.
 *  
 * @param data_d            Output k-space sample data value, complex, sorted due to precomputation
 * @param crds_d            k-space sample coordinate (non-cartesian), linearized array (x1,x2,x3,...,xn,y1,y2,y3,...,yn,z1,z2,z3,...zn)
...
/**
 * \brief Forward gridding convolution implementation on GPU using sector load balancing. 
 *
 * Performs the forward gridding convolution step on the GPU, thus the interpolation 
 * from uniform oversampled grid positions to non-uniform sampled k-space data points. 
 * 
...
/**
 * \brief Forward gridding convolution implementation on GPU using sector load balancing and textures. 
 *
 * Performs the forward gridding convolution step on the GPU, thus the interpolation 
 * from uniform oversampled grid positions to non-uniform sampled k-space data points. 
 * 
...
/** \brief Scale each element of the input data by the value of the density compensation function for the corresponding sample point. 
  *
...
/** \brief Apply the deapodization function to the image using a precomputed deapodization array
  *
  * @param imdata_d Image data
...
/** \brief Apply the deapodization function to the image in the forward transformation
  *
  * @param imdata_d Image data
  * @param gi_host Info struct with meta information
  */
...
/** \brief Apply the deapodization function to the image in the forward transformation using a precomputed deapodization array
  *
  * @param imdata_d Image data
  * @param deapo_d Precomputed deapodization
  * @param gi_host Info struct with meta information
