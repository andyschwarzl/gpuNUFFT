...
namespace gpuNUFFT
{
   ...
   * 
   * The GpuNUFFTOperator is basically instantiated by the gpuNUFFT::GpuNUFFTOperatorFactory. 
   * The Operator is provided with all the information needed in order to perform the gridding operations. 
   * 
  ...
  protected:
    /** \brief gpuNUFFT::OperatorType classifier. Value according to sub-class implementation. */
    ...
    /** \brief Array of indices defining the sort order/mapping of input k-space data to sectors
      * 
      * Mapping/assignment of data index to according sector is defined by the dataIndices array
    ...
    /** \brief Virtual method to perform precomputation of all neccessary meta information used in the gridding steps. 
    * 
    * @see gpuNUFFT::GpuNUFFTInfo
    */
    ...
    /** \brief Virtual adjoint convolution call, which can be used by sub-classes to add behaviour to the gridding steps
     *
     */
  ...
  private: 
    ...
    /** \brief Function to allocate all neccessary device memory used by the GriddingOperator. */
