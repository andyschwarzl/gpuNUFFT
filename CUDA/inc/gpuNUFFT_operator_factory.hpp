...
namespace gpuNUFFT
{
  /** \brief Manages the initialization of the GpuNUFFT Operator and its sub classes.
   * 
   ...
   *
   * The factory defines how the operator is going to process (load balancing and/or 
   * texture interpolation). 
   * 
   * Sector mapping: 
   * 
   * The grid is split into multiple subunits (sectors) in order to enable a parallel 
   * processing strategy. The sector size in each dimension is defined by the parameter 
   * 'sectorWidth', e.g. a grid of (256,256,256) is split into (32,32,32) sectors by 
   * a sector width of 8. 
   *
  ...
  public:
    ...
    /** \brief Create GpuNUFFT Operator. 
      * 
      * Based on the k-space trajectory (x,y,(z))  and the given gridding parameters the 
    ...
    /** \brief Load GpuNUFFT Operator from previously computed mappings. 
      * 
      * Based on a previously performed mapping the GpuNUFFTOperator can be loaded. 
      * 
      * @param kSpaceTraj             coordinate array of sample locations
  ...
  protected:
    /** \brief Assign the samples on the k-space trajectory to its corresponding sector 
      * 
    ...
    /** \brief Sort array of values and keep index information for further processing. 
      *
    ...
    /** \brief Compute the boundaries of the assigned dataIndices per sector element.
      *
      ...
      * 
      *  The boundaries are computed as subsequently [start,end) pairs, like [0,3,5,6]
      *  So: 
    ...
    /** \brief Method to create a new GpuNUFFTOperator or a sub class depending on the parameters. 
      *  
      * Depending on the initializiation of the factory different GpuNUFFTOperators may be 
      * allocated:  
      *
