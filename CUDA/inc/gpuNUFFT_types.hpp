...
namespace gpuNUFFT
{
  ...
  */
  ...
  /** \brief Array container consisting of raw data and gpuNUFFT::Dimensions descriptor.
   *
   */
  template <typename T>
  struct Array
  ...
  enum OperatorType
  {
    ...
    BALANCED,
    /** \brief Gridding Operator using load balancing and Texture interpolation on GPU. */
    BALANCED_TEXTURE
  };

  /** \brief Struct containing meta information of the current Gridding Problem. Used in most GPU operations.  
    * 
    ...
    * context.  
    * 
    * Initialization is performed in gpuNUFFT::GpuNUFFTOperator::initGpuNUFFTInfo() function. 
    * 
  ...
  struct GpuNUFFTInfo 
  {
    ...
    /**\brief Maximum index per dimension of padded sector (sector_pad_width - 1).*/
    int sector_pad_max;
    /**\brief Total amount of elements in one padded sector.*/
    int sector_dim;
    /**\brief Offset to zero position inside padded sector (sector_pad_width / 2). 
     * Used in combination with the sector center in order to get to the starting index (bottom left of the front slice) 
     */
    int sector_offset;
