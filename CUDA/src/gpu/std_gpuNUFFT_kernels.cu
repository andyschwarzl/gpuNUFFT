...
__global__ void minMaxBoundsKernel(DType* crds, IndType* sectors, IndType* sector_centers,uchar2* minmax_bounds, int N)
{
  int sec = blockIdx.x;
...
__global__ void minMaxBoundsKernel2D(DType* crds, IndType* sectors, IndType* sector_centers,uchar2* minmax_bounds, int N)
{
  int sec = blockIdx.x;
...
void computeMinMaxBounds(DType* crds_d, IndType* sectors_d, IndType* sector_centers_d,uchar2* minmax_bounds_d, gpuNUFFT::GpuNUFFTInfo* gi_host)
{
  ...
  dim3 block_dim(THREAD_BLOCK_SIZE);
  if (gi_host->is2Dprocessing)
    minMaxBoundsKernel2D<<<grid_dim,block_dim>>>(crds_d,sectors_d,sector_centers_d,minmax_bounds_d, gi_host->sector_count);
  else
    minMaxBoundsKernel<<<grid_dim,block_dim>>>(crds_d,sectors_d,sector_centers_d,minmax_bounds_d, gi_host->sector_count);
}
