...
TEST(TestGPUGpuNUFFTDeapo,2D_32_32_4)
{
  ...
  DType2* data = (DType2*) calloc(2*data_entries*n_coils,sizeof(DType2)); //2* re + im
  unsigned data_cnt = 0;
  ...
  for (unsigned chn = 0; chn < n_coils; chn++)
  {
    ...
    //EXPECT_NEAR(gdata[chnOffset + computeXY2Lin(0,0,imgDims)].x,0.016846,epsilon);
    EXPECT_NEAR(gdata[computeXY2Lin(10,0,imgDims)].x,gdata[chnOffset + computeXY2Lin(10,0,imgDims)].x,epsilon);
    EXPECT_NEAR(gdata[computeXY2Lin(0,5,imgDims)].x,gdata[chnOffset + computeXY2Lin(0,5,imgDims)].x,epsilon);
    EXPECT_NEAR(gdata[computeXY2Lin(4,4,imgDims)].x,gdata[chnOffset + computeXY2Lin(4,4,imgDims)].x,epsilon);
    EXPECT_NEAR(gdata[computeXY2Lin(15,14,imgDims)].x,gdata[chnOffset + computeXY2Lin(15,14,imgDims)].x,epsilon);
  }
...
TEST(TestGPUGpuNUFFTDeapo,2D_32_32_4_B1)
{
  int kernel_width = 3;
  //oversampling ratio
  float osr = 2.0f;//DEFAULT_OVERSAMPLING_RATIO;
  ...
  DType2* b1 = (DType2*) calloc(im_width*im_width*n_coils,sizeof(DType2)); //2* re + im
  unsigned b1_cnt = 0;
  ...
  gpuNUFFT::GpuNUFFTOperatorFactory factory(false,true,true); 
  gpuNUFFT::GpuNUFFTOperator *gpuNUFFTOp = factory.createGpuNUFFTOperator(kSpaceData, densData, b1Data, kernel_width,sector_width,osr,imgDims);
  ...
  EXPECT_NEAR(gdata[computeXY2Lin(0,0,imgDims)].x,0.00999,epsilon);
