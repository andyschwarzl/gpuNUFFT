    ...
    if (adjoint == true)
        ...
        if (single_coil == true)
            ...
            for coil = 1 : A.numCoils,
                ...
                tmp = mex_gridding3D_sparse(single(b_il(coil_start:coil_end)),single(ipk.sn),single(A.imageDim),single(1),single(idx),single(weight),single(ipk.Kd), uint32(bp_vxidx), bp_midx, bp_weight,single([it lambda^2 devnum tol verbose]));
            ...
            res = mex_gridding3D_sparse(single(b_il),single(A.imageDim),single(ipk.sn),single(b_coils),single(idx),single(weight),single(ipk.Kd), uint32(bp_vxidx), bp_midx, bp_weight,single([it lambda^2 devnum tol verbose]));
        ...
        res = mex_gridding3D_sparse_forw(single(img_il),single(A.imageDim),single(ipk.sn),single(1),single(idx),single(weight),single(ipk.Kd), uint32(bp_vxidx), bp_midx, bp_weight,single([it lambda^2 devnum tol verbose]));
